<script>
  ...
  function cellHtml(value) {
    ...
    while ((m = URL_RE.exec(s)) !== null) {
      ...
      parts.push(`<a href="${eu}" target="_blank" rel="noopener noreferrer" class="url-link">${eu}</a>`)
  ...
  let displayRows = []
  $: {
    const { filters: f, sortCol: sc, sortDir: sd } =
    ...
    displayRows = r.map(row => {
      const bv = alignRowsBy ? String(row[alignRowsBy] ?? '') : effectiveRowKey(row)
      if (bv !== lastBandVal) { band = !band; lastBandVal = bv }
  ...
  function startResize(e, key) {
    ...
    resizing = { key, startX: e.clientX, startWidth: colWidths[key] ?? th.getBoundingClientRect().width }
  ...
  function onMousemove(e) {
    ...
    colWidths = {
      ...colWidths,
      [resizing.key]: Math.max(40, resizing.startWidth + (e.clientX - resizing.startX)),
    }
  }
        ...
        <tr>
          {#each effectiveCols as col}
            ...
            >
              ...
              <div class="col-resizer" on:mousedown|preventDefault={e => startResize(e, col.key)} />
            </th>
          {/each}
        </tr>
      </thead>
  ...
  >
    ...
    {#each Object.keys(panelState) as val}
      <label class="filter-row">
        <input type="checkbox" bind:checked={panelState[val]} on:change={syncFilter} />
...
<style>
  ...
  .sort-icon { font-size: 9px; opacity: 0.45; flex-shrink: 0; }
  ...
  .filter-btn:hover         { color: var(--text-secondary); background: rgba(255,255,255,0.06); }
  .filter-btn.filter-active { color: var(--green); }
  ...
  .col-resizer:hover { background: var(--green-border); }
  ...
  tr.band td { background: rgba(255,255,255,0.02); }

  tr.clickable         { cursor: pointer; }
  tr.clickable:hover td { background: var(--green-glow); }
  tr.highlighted td    { background: var(--green-dim) !important; }
  ...
  .filter-row input { accent-color: var(--green); cursor: pointer; flex-shrink: 0; }

  .filter-all { font-weight: 600; color: var(--text-secondary); }
  ...
  .filter-divider { height: 1px; background: var(--border); margin: 3px 0; }
</style>
